...
wriggle_dev_log_handler = handlers.TimedRotatingFileHandler(
...
wriggle_dev_log_handler.setFormatter(_dev_formatter)
wriggle_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

wriggle_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

wriggle_usr_log_handler.setLevel(logging.INFO)
...
wriggle_usr_log_handler.setFormatter(_usr_formatter)
...
wriggle_console_log_handler = logging.StreamHandler(sys.stdout)
wriggle_console_log_handler.setFormatter(_console_formatter)
wriggle_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    wriggle_usr_log_handler.addFilter(warning_filter)
    wriggle_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    wriggle_usr_log_handler.removeFilter(warning_filter)
    wriggle_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    wriggle_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(wriggle_dev_log_handler)

    log.addHandler(wriggle_console_log_handler)

    log.addHandler(wriggle_usr_log_handler)
